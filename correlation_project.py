...
#Use temperature cuts to separate AllStar data
#into red and blue
allstar = star_list
a = allstar['TEFF'] <= 3500
r = allstar[a]
...
#Plot postitions of Red and Blue stars
...
#Get number of Red and Blue stars
print("SDSS data size:")
print("  red stars: ", len(R))
print("  blue stars:", len(B))

#Calculate the correlation of Red and Blue stars
...
R_gaia = pd.read_csv('data/gaia_red.csv')
B_gaia = pd.read_csv('data/gaia_blue.csv')
...
#Make plots of stellar postitions in galactic coordinates
...
#Repeat correlation calculations and plots for SDSS Disk
bins = np.linspace (0.1, 10, 11) # edges for the 10 bins to evaluate
...
#Repeat correlation calculations and plots for Gaia Disk
bins = np.linspace (0.1, 10, 11) # edges for the 10 bins to evaluate
...
#Repeat correlation calculations and plots for SDSS Halo
bins = np.linspace (0.1, 10, 11) # edges for the 10 bins to evaluate
...
bins = np.linspace (0.1, 10, 11) # edges for the 10 bins to evaluate
corr_R_gaia_halo, R_gaia_err, R_boot = bootstrap_two_point_angular(R_gaia_halo_l, 
                                                                   R_gaia_halo_b, 
                                                                   bins, 
                                                                   method='landy-szalay',
                                                                   Nbootstraps=10)
corr_B_gaia_halo, B_gaia_err, B_boot = bootstrap_two_point_angular(B_gaia_halo_l, 
                                                                   B_gaia_halo_b, 
                                                                   bins, 
                                                                   method='landy-szalay',
                                                                   Nbootstraps=10)
...
plt.errorbar(bin_centers, corr_R_gaia_halo, R_gaia_err,
                fmt='.k', ecolor='gray', lw=1)
plt.errorbar(bin_centers, corr_B_gaia_halo, B_gaia_err,
                fmt='.k', ecolor='gray', lw=1)
plt.xlim(0,9.5)
...
corr_gaia_err = [R_gaia_err, B_gaia_err]
boot = [R_boot, B_boot]
